import {apply,empty,find,term,availability,generate,checkResult} from './motor.mjs';
...
function render(){
  ...
  $('#mode-description').textContent=mode==='creator'?'Construye libremente. Las operaciones conservan el valor; crear, eliminar o elevar cambia la expresión.':'Simplifica con operaciones exactas. No puedes añadir, borrar ni crear potencias nuevas.';
...
function check(){if(mode!=='solve')return;try{const result=checkResult(state,problem.state,problem.requireScientific);announce(result.message,!result.ok);$('#status').classList.toggle('success',result.ok);}catch(e){announce(e.message,true);}}
...
function newTerm(){return {term:term($('#coefficient').value,{x:$('#var-x').value,y:$('#var-y').value,z:$('#var-z').value}),side:$('#new-side').value};}
function updateNew(){try{const v=newTerm();$('#term-preview').innerHTML=formula(v.term);$('#preview-side').textContent=v.side==='top'?'Se añadirá al numerador.':'Se añadirá al denominador.';}catch{$('#term-preview').textContent='Completa el factor para ver la vista previa.';$('#preview-side').textContent='';}}
...
const shortcuts={n:'new',p:'power',e:'evaluate',c:'combine',x:'cancel',s:'positive',z:'undo',ArrowUp:'up',ArrowDown:'down',ArrowLeft:'left',ArrowRight:'right',Delete:'delete',Backspace:'delete'};
