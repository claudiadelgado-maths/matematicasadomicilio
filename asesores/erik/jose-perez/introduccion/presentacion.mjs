import { topics } from './contenido.mjs';
if (topics.some(topic => location.hash === `#${topic.id}`)) location.replace(new URL(`recorrido/index.html${location.hash}`, location.href));
...
form.addEventListener('submit', (event) => {
  ...
  const phrase = `Hola, soy ${name}, tengo ${age} años, mi carrera u ocupación es ${occupation} y me gusta ${hobby}.`;
