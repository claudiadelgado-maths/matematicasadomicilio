...
6. Añade la tarjeta al índice `/biblioteca/temas/`.
7. Ejecuta `npm run catalogo` y `npm run validar`.

El `tema.json` debe declarar `id`, `titulo`, `descripcion`, `categoria`, `nivel`, `etiquetas`, `estado`, `ruta` y booleanos de componentes que coincidan con el disco.
